...
class CircuitBreaker:
    ...
    async def execute(
        self, provider: str, operation: Callable[[], Any], fallback: Callable[[], Any] | None = None
    ) -> Any:
        """
        ...
        if state == CircuitState.OPEN:
            ...
            logger.warning("Circuit breaker OPEN", provider=provider, retry_after=retry_after)
        ...
        async with self._lock:
            ...
            if current_state == CircuitState.HALF_OPEN:
                ...
                if stats.successes >= self.config.success_threshold:
                    ...
                    logger.info(
                        "Circuit breaker CLOSED", provider=provider, reason="recovery_successful"
                    )
    ...
    async def _record_failure(self, provider: str, error: Exception) -> None:
        ...
        async with self._lock:
            ...
            if current_state == CircuitState.HALF_OPEN:
                ...
                self._half_open_calls[provider] = 0
                logger.warning(
                    "Circuit breaker re-OPENED", provider=provider, reason="half_open_failure"
                )
                ...
                if stats.failures >= self.config.failure_threshold:
                    ...
                    stats.opened_at = time.time()
                    logger.error(
                        "Circuit breaker OPENED", provider=provider, failures=stats.failures
                    )
...
def with_circuit_breaker(provider: str):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            return await breaker.execute(provider, lambda: func(*args, **kwargs))

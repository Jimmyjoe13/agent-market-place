...
def validate_prompt_length(prompt: str, max_length: int = 4000) -> Tuple[bool, str]:
    ...
    if len(prompt) > max_length:
        return False, f"System prompt exceeds maximum length of {max_length} characters (got {len(prompt)})"
...
def validate_system_prompt(
    ...
    if is_dangerous and severity == "blocked" and block_injection:
        return False, f"Potential prompt injection detected. Blocked patterns found.", sanitized
...
def check_prompt_complexity(prompt: str) -> dict:
    ...
    if newlines > 10: score += 1
    if newlines > 30: score += 1
    if has_code: score += 2
    if estimated_tokens > 500: score += 1
